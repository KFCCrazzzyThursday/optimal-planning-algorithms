...
function random_restarts(loss, hard_lower, soft_lower, soft_upper, hard_upper, n_restart; 
    ...
    function wrap_loss(x)
        ...
        loss(squashed)  # tiny regularization stabilizes optimization
    end
    ...
    function do_opt(x0)
        ...
        try
            ...
            res = optimize(wrap_loss, unsquash!(box, copy(x0)), algorithm,
                Optim.Options(;g_tol=g_tol, iterations=iterations); autodiff=:forward)
...
function Distributions.fit(::Type{M}, trials::Vector{Trial}; 
            method=:bfgs, n_restart=500, opt_kws...) where M <: AbstractModel
    ...
    function make_loss(z)
        x -> begin
            ...
            nll = -logp(L, model)  # 计算 nll
            # if COUNT % 50 == 0 
            #     println("Current model parameters: ")
            #     println("  cost: ", model.cost)
            #     println("  β_select: ", ForwardDiff.value(model.β_select))
            #     println("  β_term: ", ForwardDiff.value(model.β_term))
            #     println("  β_expand: ", ForwardDiff.value(model.β_expand))
            #     println("  ε: ", ForwardDiff.value(model.ε))
            #     println("nll: ", ForwardDiff.value(nll),"\n")  # 提取并打印 nll 的标量值
            # end
            return nll
        end
    end
    ...
    results, elapsed = @timed map(combinations(space)) do z
        ...
        opt = begin
            if method == :samin
                x0 = soft_lower .+ rand(length(hard_lower)) .* (soft_upper .- soft_lower)
                optimize(loss, hard_lower, hard_upper, x0, SAMIN(verbosity=0), Optim.Options(iterations=10^6))
            elseif method == :bfgs
                t = trials[1]
                id = "$(name(M))-$(t.wid)-$(t.i)"
                random_restarts(loss, all_bounds..., n_restart; id=id, opt_kws...)
            end
        end

...
function write_mdps(ids)
    ...
    # # 序列化未解决文件列表
    # serialize("tmp/unsolved_pseudo", unsolved)
    # println("Serialized unsolved files to tmp/unsolved_pseudo")
    if isfile("tmp/unsolved_pseudo")
        unsolved = deserialize("tmp/unsolved_pseudo")
        println("Loaded (faked) unsolved files from tmp/unsolved_pseudo: ", unsolved)
        return unsolved
    else
        println("No unsolved file found in tmp/unsolved_pseudo")
        return String[]
    end
...
@everywhere function solve_mdp(i::String)
    ...
    # if getpid() % 4 == 0
    #     println("Process with PID divisible by 4 detected. Delaying for 4 hours.")
    #     sleep(5 * 60 * 60)  # 延迟30分钟
    # end

    m = deserialize("mdps/withcost/$i")
    if isfile("mdps/V_2/$i")
    ...
    serialize("mdps/V_2/$i", V)
    V = nothing
    GC.gc()
end

@everywhere do_job(id::String) = solve_mdp(id)
@everywhere do_job(idx::Int) = solve_mdp(deserialize("tmp/unsolved_pseudo")[idx])
do_job(jobs) = pmap(solve_mdp, deserialize("tmp/unsolved_pseudo")[jobs])

...
function load_trials(experiment)::Dict{String,Vector{Trial}}
    println("Loading","../data/$experiment/trials.json")
...
# function load_trials(experiment)::Dict{String,Vector{Trial}}
#     data = open(JSON.parse, "../data/$experiment/trials.json")
#     println("hahahahha - data: ", data)  # 打印整个数据结构

#     map(data) do wid, tt
#         println("Processing: wid = ", wid, ", tt = ", tt)  # 打印每个 wid 和 tt

#         # 检查 tt 是否是一个非空数组，并打印出它的长度和类型
#         if !(tt isa Vector)
#             println("Error: tt is not a Vector, it is: ", typeof(tt))  # 错误检查
#         elseif length(tt) < 2
#             println("Error: tt does not have enough elements, length: ", length(tt))  # 检查长度
#             println("Problematic tt: ", tt)  # 打印导致错误的部分
#         end
